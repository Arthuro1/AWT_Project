...
import {AUTH_ERROR, AUTH_REGISTER, AUTH_LOG_IN, AUTH_LOG_OUT, FILTER_SELECTED, DATA_REQUEST, DATA_REQUEST_ERROR} from "./types";
...
export const setFilter = (newFilter) => {
 ...
 export const searchForLecture = (data) => {
     return async dispatch => {
         try {
             const res = await axios.get('http://localhost:5000/dashboard/books/search-by-lecture');
             console.log('res', res);
             dispatch({
                 type: DATA_REQUEST,
                 payload: res.data
